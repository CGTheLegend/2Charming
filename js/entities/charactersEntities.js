...
game.girlEntity = me.ObjectEntity.extend({
	...
	init : function(x, y, settings) {// a constructor
		...
		this.setVelocity(walking_speed, girl_jump_speed);// set the default (horizontal, vertical) speed,
														 // horizontal : results in movement speed, vertical : jumps higher
	...
	update : function() {//updating girl's position
		...
		if ((game.data.control_girl == true )) {
			...
			var targetRight = new me.Vector2d(this.pos.x +300, this.pos.y);	
...
game.boyEntity = me.ObjectEntity.extend({
	...
	update : function() {//updating boy's position
		...
		//	this.attacking = false;
			...
			else if (me.input.keyStatus('dash') && !this.cooling) {	
			...
			if ((this.jumping || this.falling )  && (!stand || this.dashing) ) {
                this.renderable.setCurrentAnimation("jump");
            }
		
		     if (me.input.isKeyPressed("attack") &&  !this.attacking) { 
		     //	this.attacking = true;
			...
			else {//if (this.falling || !this.attacking){
...
{
	if ( res.obj.name == "droppable" && res.x && !res.y && !character.falling)	{
...
function enemyCollision (res, character){
	...
	if ((res.y > 0) && !character.jumping) {// check if we jumped on it, if yes, bounce (fouce jump)
...
{
	if ( character.pos.y+100 > me.game.currentLevel.rows * me.game.currentLevel.tileheight  || game.data.health == 0) // fall and out of scope
 			{	
 				broken = false;
            	me.game.viewport.fadeIn("#000", 500, function () {
